...
def _upload_file_dict_to_s3(file_dict, key_dict, path, key_name):
    '''
    ...
    k = Key(bucket)
    k.key = 'xqueue/{path}/{key}'.format(path=path, key=key_name)
...
def _upload_to_s3(file_to_upload, path, key_name):
    '''
